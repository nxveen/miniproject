...
print("locations $start_loc $end_loc");
    var prevDeps = await _hikers.doc(start_loc).get();
    Map? data = {};
    ...
    if (prevDeps.exists) {
      ...
      if (data !=null) {
        ...
        if (data["departures"] != null) {
          await _hikers.doc(start_loc).update({"departures": [...data["departures"], user_id]});
        } else {
          await _hikers.doc(start_loc).update({"departures": [user_id]});
        }
      }
    } else {
      print("prevData donot exists");
      await _hikers.doc(start_loc).set({"departures": [user_id], "arrivals": []});
    }


    var prevArrv = await _hikers.doc(end_loc).get();
    Map? arrData = {};
    if (prevDeps.exists) {
      arrData = prevArrv.data();
      if (arrData != null) {
        if (arrData["arrivals"] != null) {
          await _hikers.doc(end_loc).update(
              {"arrivals": [...arrData["arrivals"], user_id]});
        } else {
          await _hikers.doc(end_loc).update({"arrivals": [user_id]});
        }
      }
    } else {
      await _hikers.doc(end_loc).set({"departures": [], "arrivals": [user_id]});
    }
    return "success";
  }

  Future<String> searchHost({
  ...
  Future<String> removeRideRequest({
    ...
    if (data !=null) {
      if (data["departures"] != null) {
        ...
        await _hikers.doc(start_loc).update({"departures": data["departures"]});
      } else {
        await _hikers.doc(start_loc).update({"departures": []});
      }
    }
    ...
    if (arrData !=null) {
      if (arrData["arrivals"] != null) {
        ...
        await _hikers.doc(end_loc).update({"arrivals": arrData["arrivals"]});
      } else {
        await _hikers.doc(end_loc).update({"arrivals": []});
      }
    }
    return "success";
  }
